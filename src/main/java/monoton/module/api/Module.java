...
import monoton.module.impl.player.ClientSounds;
import monoton.module.settings.Configurable;
...
public abstract class Module extends Configurable implements IMinecraft {
    ...
    public void toggle() {
        ...
        Manager.NOTIFICATION_MANAGER.add((state ? name + " " + "was enabled!" : name + " "  +"disabled!"), "Function Debug", 3, this.category);

        ClientSounds clientSounds = Manager.FUNCTION_MANAGER.clientSounds;

        if (clientSounds.state) {
            if (!state) {
                SoundUtils.playSound("Function_OFF.wav", Manager.FUNCTION_MANAGER.clientSounds.voulme.getValue().floatValue());
            } else {
                SoundUtils.playSound("Function_ON.wav", Manager.FUNCTION_MANAGER.clientSounds.voulme.getValue().floatValue());
            }
        }
    }
