...
public class ProjectionUtils implements IMinecraft {
    ...
    public static Vector2f project2(double x, double y, double z) {
        Vector3d camera_pos = mc.getRenderManager().info.getProjectedView();
        Quaternion cameraRotation = mc.getRenderManager().getCameraOrientation().copy();
        cameraRotation.conjugate();
        ...
        return calculateScreenPosition2(result3f, fov, aspectRatio);
    }
    ...
    private static Vector2f calculateScreenPosition2(Vector3f result3f, double fov, float aspectRatio) {
    ...
    private static Vector2d calculateScreenPosition(Vector3f result3f, double fov) {
        ...
        if (result3f.getZ() < 0.0F) {
            return new Vector2d(
                    -result3f.getX() * scaleFactor + mc.getMainWindow().getScaledWidth() / 2.0F,
                    mc.getMainWindow().getScaledHeight() / 2.0F - result3f.getY() * scaleFactor
            );
        }
        return null;
